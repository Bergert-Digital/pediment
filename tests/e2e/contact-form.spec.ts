...
test('contact form submission shows success and creates a submission row', async ({ page }) => {
  ...
  const url = createPageWithContent(CONTACT_SLUG, 'Contact test', '<!-- wp:pediment/contact-form /-->');
...
test('honeypot triggers rejection (silent)', async ({ page }) => {
  ...
  const url = createPageWithContent(HONEYPOT_SLUG, 'Honeypot test', '<!-- wp:pediment/contact-form /-->');
