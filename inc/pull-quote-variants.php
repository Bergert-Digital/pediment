 ...
 *
 *   add_filter( 'pediment_pull_quote_variants', fn( $v ) => array_diff( $v, [ 'testimonial' ] ) );
 *
 ...
 */
function pediment_pull_quote_variants() {
	$defaults = array( 'default', 'testimonial' );
	$variants = apply_filters( 'pediment_pull_quote_variants', $defaults );
...
add_action(
	...
	function () {
		wp_add_inline_script(
			'wp-blocks',
			'window.pedimentPullQuoteVariants = ' . wp_json_encode( pediment_pull_quote_variants() ) . ';',
			'after'
		);
	}
);
