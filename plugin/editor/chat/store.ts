...
const reducer = (
	...
	switch ( action.type ) {
		...
		case 'SET_PENDING_USER_MESSAGE':
			return { ...state, pendingUserMessage: action.pendingUserMessage };
		case 'SET_ERROR':
...
const actions = {
	setConversation: ( conversation: Conversation | null ): Action => ( {
		type: 'SET_CONVERSATION',
		conversation,
	} ),
	setStreaming: ( streaming: ChatMessage | null ): Action => ( {
		type: 'SET_STREAMING',
		streaming,
	} ),
	...
	setPendingUserMessage: (
		pendingUserMessage: ChatMessage | null
	): Action => ( { type: 'SET_PENDING_USER_MESSAGE', pendingUserMessage } ),
