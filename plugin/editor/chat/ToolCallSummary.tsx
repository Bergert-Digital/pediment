...
export default function ToolCallSummary({ calls }: { calls: any[] }) {
  ...
  return (
    <div className="pediment-chat__tools">
      <button type="button" className="pediment-chat__tools-toggle" onClick={() => setOpen(!open)}>
        {open ? '▾ ' : '▸ '}{label}
      </button>
      {open && (
        <pre className="pediment-chat__tools-detail">{JSON.stringify(calls, null, 2)}</pre>
      )}
    </div>
  );
}
