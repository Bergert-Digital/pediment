...
export default function useChatTurn() {
	const streaming = useSelect(
		( s ) =>
			( s( STORE_NAME ) as any ).getStreaming() as ChatMessage | null,
		[]
	);
	const error = useSelect(
		( s ) => ( s( STORE_NAME ) as any ).getError() as string | null,
		[]
	);
	const {
		setStreaming,
	...
	const stop = useCallback( () => {
		if ( streaming ) {
			...
			if ( streaming.id > 0 ) {
				apiFetch( {
					path: `/pediment/v1/chat/turns/${ streaming.id }`,
					method: 'DELETE',
				} ).catch( () => {} );
			}
			aborted = true;
				...
				onTerminal: async () => {
					...
					try {
						const conv = await apiFetch< Conversation >( {
							path: `/pediment/v1/chat/conversations?post_id=${ args.postId }`,
							method: 'GET',
						} );
