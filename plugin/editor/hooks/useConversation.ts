...
export default function useConversation( postId: number | null ) {
	const conv = useSelect(
		( s ) =>
			( s( STORE_NAME ) as any ).getConversation() as Conversation | null,
		[]
	);
	...
	const load = useCallback( async () => {
		...
		if ( inflight && lastLoadedPostId === postId ) {
			return inflight;
		}
		...
		inflight = ( async () => {
			try {
				const data = await apiFetch< Conversation >( {
					path: `/pediment/v1/chat/conversations?post_id=${ postId }`,
					method: 'GET',
				} );
	...
	const clear = useCallback( async () => {
		...
		await apiFetch( {
			path: `/pediment/v1/chat/conversations/${ conv.id }`,
			method: 'DELETE',
		} );
