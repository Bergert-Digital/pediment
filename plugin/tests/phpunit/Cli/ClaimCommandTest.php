...
class ClaimCommandTest extends WP_UnitTestCase {
	...
	public function test_render_prints_the_claim_report() {
		...
		$out = ClaimCommand::render( new ClaimResult( $plan, false, '/srv/theme/seed/manifest.php', [] ) );
	 ...
	 * render() that silently dropped the errors (e.g. rebuilding the
	 * ClaimResult it hands Reporter::claimText() with a hardcoded `[]`) would
	 * still pass it. The __invoke() method decides whether to call
	 * WP_CLI::error() based on this exact array, so its presence in the
	 * rendered text is what a human operator — and this test — relies on to
	 * notice a partially-applied claim.
	 */
	public function test_render_includes_errors_when_present() {
		$out = ClaimCommand::render(
			new ClaimResult( new Plan(), true, '/srv/theme/seed/manifest.php', [ 'home|: post 12 already carries a seed key — nothing was written for this entry.' ] )
		);
