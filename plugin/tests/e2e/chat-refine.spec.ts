...
test( 'quick action shortens the selected paragraph via chat', async ( {
	page,
} ) => {
	await login( page );
	...
	await page.keyboard.type(
		'A long paragraph that needs shortening, several sentences indeed.'
	);

	// Select the paragraph (click in it).
	await editor
		.locator( 'p.wp-block-paragraph', { hasText: 'A long paragraph' } )
		.click();
	...
	await expect(
		editor.locator( 'p.wp-block-paragraph', { hasText: 'Short.' } )
