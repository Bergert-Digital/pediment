...
test.describe( 'blog landing (/blog/)', () => {
	test( 'renders heading band and insight cards with category badge', async ( {
		page,
	} ) => {
		...
		await expect(
			page.getByRole( 'heading', {
				level: 1,
				name: /latest thinking from the team/i,
			} )
		).toBeVisible();

		// Grid: at least one post card
		const firstCard = page
			.locator( '.is-style-insights-grid .wp-block-post-template > li' )
			.first();
	...
	test( 'no client-side filter row on the paginated landing', async ( {
		page,
	} ) => {
		await page.goto( '/blog/' );
		// The curated block's filter — must not appear here.
		await expect(
			page.locator( '.starter-blog-index__filter' )
		).toHaveCount( 0 );
	} );
} );
