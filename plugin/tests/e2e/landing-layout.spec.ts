...
test.describe( 'landing layout (1440×900)', () => {
	...
	test( 'cta: bounding box width matches max-width (border-box)', async ( {
		page,
	} ) => {
		await page.goto( '/' );
	...
	test( 'testimonial: grid is alignwide and shows two cards', async ( {
		page,
	} ) => {
		await page.goto( '/' );
		const grid = page.locator( '.starter-testimonial-grid' ).first();
		await grid.scrollIntoViewIfNeeded();
		const gw = await grid.evaluate( ( el ) =>
	...
	test( 'services: section-head and feature-grid share the same left edge', async ( {
		page,
	} ) => {
		await page.goto( '/' );
		...
		const headX = await head.evaluate( ( el ) =>
			Math.round( el.getBoundingClientRect().left )
		);
		const gridX = await grid.evaluate( ( el ) =>
			Math.round( el.getBoundingClientRect().left )
		);
