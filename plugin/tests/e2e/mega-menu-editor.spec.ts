...
test.describe( 'mega-menu editor (entity context)', () => {
	...
	test( 'LINCHPIN: updateBlockAttributes preserves mega-menu clientId in wp_navigation entity', async ( {
		...
		const initialClientId: string =
			( await wrapper.getAttribute( 'data-block' ) ) ?? '';
	...
	test( 'sidebar form: Add column reveals Column 2 panel', async ( {
		page,
	} ) => {
		navId = createNavigationEntityWithContent(
			'mega-form',
		...
		const canvas = page.frameLocator( 'iframe[name="editor-canvas"]' );
		await expect(
			canvas.locator( '.starter-mega-menu' ).first()
		).toBeVisible( {
			timeout: 20000,
		} );
...
test.describe( 'mega-menu editor (page context — read-only)', () => {
	test( 'Inspector form is hidden and editing mode is disabled', async ( {
		page,
	} ) => {
