...
test( 'chat composer attaches an image and persists its thumbnail', async ( {
	page,
} ) => {
	await login( page );
	...
	await expect( sidebar.locator( '.pediment-chat__thumb img' ) ).toBeVisible(
		{
			timeout: 5_000,
		}
	);
	...
	await expect(
		sidebar.locator( '.pediment-chat__msg-images img' ).first()
	).toBeVisible( { timeout: 15_000 } );

	// The mock still applies its tool call to the canvas.
	const editor = await canvas( page );
	await expect(
		editor.locator( 'p.wp-block-paragraph', {
