...
test( 'chat panel inserts a paragraph from mock fixture', async ( {
	page,
} ) => {
	await login( page );
	...
	await expect(
		sidebar.getByText( 'Add a paragraph that says hi' )
	).toBeVisible( {
		timeout: 5_000,
	} );

	// The mock's assistant prose streams in.
	await expect( sidebar.getByText( /adding a paragraph/i ) ).toBeVisible( {
		timeout: 15_000,
	} );
